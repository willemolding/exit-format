...
library ExitFormat {
    ...
    //   This might specify how to transfer this particular asset (e.g. target an "ERC20.transfer"' method)
    //   It is also how contracts supporting multiple tokens are supported (e.g. ERC1155)
    ...
    function executeSingleAssetExit(
        ...
        for (uint256 j = 0; j < singleAssetExit.allocations.length; j++) {
            ...
            if (asset == address(0)) {
                ...
                if( _metadata_.length == 0 || _metadata_[0] == bytes1(uint8(ExitMetadataType.ERC20)) ) {
                    IERC20(asset).transfer(destination, amount);
                }
                // 1 => ERC1155 
                else if (_metadata_[0] == bytes1(uint8(ExitMetadataType.ERC1155)) ) {
                    (, ERC1155ExitMetadata memory metadata) = abi.decode(_metadata_, (ExitMetadataType, ERC1155ExitMetadata));
                    IERC1155(asset).safeTransferFrom(msg.sender, destination, metadata.tokenId, amount, "");
                }
            }
            if (
